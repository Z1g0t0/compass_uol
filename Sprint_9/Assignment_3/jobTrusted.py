...
trusted = raw.select(f.col("title").alias("Titulo"), f.col("release_date").alias("Data_Lancamento"), 
                     f.col("vote_count").alias("Quantidade_Votos"), f.col("vote_average").alias("Media_Votos"), 
                     f.col("popularity").alias("Score_TMDB"), f.col("genre_ids").alias("Generos"), f.col("overview").alias("Sinopse"))
